...
export const useAuth = () => {
  ...
  const resetPassword = useCallback(async (username) => {
    try {
      ...
      console.log('🔍 Reset password per username:', username);
      
      // Prova con i diversi formati di email
      const emailFormats = [
        `${username}@dummy.com`,
        `${username}@${username}.com`,
        username // nel caso sia già un'email completa
      ];
      ...
      for (const email of emailFormats) {
        try {
          console.log('📧 Tentativo reset con email:', email);
          await sendPasswordResetEmail(auth, email);
          console.log('✅ Email di reset inviata a:', email);
          return { success: true, email };
        } catch (error) {
          console.log('❌ Reset fallito per:', email, error.message);
          lastError = error;
        }
      }
      
      // Se tutti i tentativi falliscono
      throw lastError || new Error('Nessun account trovato con questo username');
  ...
  }), [user, loading, error, login, register, logout, resetPassword]);
